...
class AeonRequest
  ...
  def self.build_requests(instances, opts = {})
    instances.map do |instance|
      ...
      request['Transaction.CustomFields.ItemFolder'] = ['2', '3'].map {|lvl|
  ...
  def self.build_top_container(json, request)
    ...
    request["Transaction.CustomFields.ItemSeries"] = json['series'].map{|s| s['level_display_string'] + ' ' + s['identifier'] + '. ' + s['display_string']}.join('; ')
